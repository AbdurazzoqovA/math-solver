...
1. **Mistake-review habit loop (shipped in code 2026-07-26)** — first-attempt misses persist on the existing saved question and feed a one-tap queue of up to five due problems. The Practice Tests navigation shows a due-review badge. The visible daily-goal/streak control was removed after owner UX review; local activity state remains available for privacy-conscious retention measurement without occupying sidebar space.
...
3. **Review scheduling depth** — extend the shipped mistake queue with topic grouping and clearer readiness cues only after the current 1→3→7→14-day loop has enough usage data.
