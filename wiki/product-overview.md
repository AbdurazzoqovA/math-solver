...
- **Pressroom-powered blog** — `/blog` renders the live Pressroom feed server-side with nine articles per page, optional covers/tags/bylines, accessible pagination, empty/error states, and five-minute cache revalidation. `/blog/[slug]` renders trusted rich article HTML, article metadata, BlogPosting structured data, optional author profiles, and a solver CTA. The footer and sidebar link to the blog; live article URLs join the sitemap automatically. At the 2026-07-26 integration check the API authenticated successfully but had zero live posts.
