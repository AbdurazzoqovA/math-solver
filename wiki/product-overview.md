...
- **Daily goal + streak** — a compact 44px sidebar row counts two distinct learning activities per local day (`solve`, completed practice, or completed review). Clicking it opens an anchored detail panel without pushing account/theme/legal controls down; the panel recommends one concrete next action—start a solve, open due review, or open practice. Meeting the goal starts/extends a streak; missing a day resets the visible streak without blocking the solver. Progress is local-first, stored separately for guests and Firebase account UIDs, and guest progress merges into the account cache at sign-in.
