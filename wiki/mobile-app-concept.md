...
- Analyzer and all 93 Flutter tests pass, including focused rating eligibility, shared-cooldown, reveal, and verification-warning regressions. Version `1.0.2 (6)` is publicly available on both the [App Store](https://apps.apple.com/us/app/ai-math-solver-mathsolver/id6799652720) and [Google Play](https://play.google.com/store/apps/details?id=io.mathsolver.app). The App Store listing went live on 2026-09-12 after the earlier Guideline 4.3(a) review issue was resolved; the Google Play listing and hardened build are live under the same `io.mathsolver.app` identity.
...
The code-controlled launch product is substantially complete and version `1.0.2 (6)` is public on both stores. The versioned gateway, private renderer, and Firestore rules reached production on 2026-07-28 with staged App Check validation. Native Google and Apple sign-in plus their Firebase/Apple provider configuration shipped on 2026-08-05. Android has passed a real Play Integrity-protected video request, and the current release includes the hardened restart/background/data-isolation work plus quiet milestone-based ratings. Remaining operational work is the APNs `.p8`, broader App Check enforcement after legitimate-traffic validation, and continued post-store-install regression testing. Widgets, Pencil scratchpad, exam packs, and Live Tutor remain later product phases rather than hidden launch claims.
...
| **1 — MVP** | Public on iOS and Android | Camera/crop/worksheet solve, hint-first verified steps, follow-up chat, private video with offline/share/optional push, guest→Firestore sync, practice + spaced mistake review, warm-up, in-app deletion, and both iOS/Android hosts. Version `1.0.2 (6)` is live on the App Store and Google Play. |
