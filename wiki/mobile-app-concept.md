...
Build a **camera-first AI math tutor** whose signature extends the shipped web engine: **per-problem generated animated video explanations**. The launch product is completely free: every non-video feature is unlimited, while verified accounts may generate exactly 10 videos per UTC day. There are no subscriptions, purchases, paid tiers, trials, or monetization prompts. Ads may be evaluated in a later release, but are not a reason to force an update. The app must NOT be a port of the web chat UI — it is a different product built around the camera, short vertical video, and a daily practice loop.
...
- One-screen first run welcomes student learners without a blocking age checkbox. The app is positioned as a normal general-audience Education app, not as a Kids Category product; the owner explicitly includes grades 5–6 in the audience. No permission is requested at launch; the immersive camera initializes only after the student taps the dominant Scan action.
...
- A solve → video → practice → review loop. Private videos autoplay chapters continuously, keep captions in a separate non-covering transcript strip, and offer opt-in Practice pauses for interactive checks and transfer. They also support 0.75–2× speed, full screen, owner-scoped offline caching, native sharing, and content-free push-on-ready deep links. Rendering runs on the private server worker: screen-off/backgrounding pauses only local polling, and resume reauthorizes and continues the same deterministic job without duplicate generation or quota use. Practice persists first misses into the 1→3→7→14-day queue.
- Native Firebase Authentication provides complete verified Email/Password create/verify/resend/reset flows, Google sign-in on iOS/Android, and Sign in with Apple on iOS with SDK-managed persistent sessions; verified accounts merge the guest notebook into owner-isolated Firestore and continue cross-device sync.
...
- Native App Store/Play rating requests are reserved for two completed learning milestones: either at least three saved solutions followed by 75%+ on a practice or mistake-review set containing four or more questions, or the fifth saved solve after its independent verification is `checked` and every step plus the final answer is visible. The solve milestone adds a two-second quiet pause. Both paths use only the system review sheet and share one local 120-day cooldown; the app never asks on launch, while streaming, before the answer is revealed, after an error/warning, or through a custom positive-sentiment gate.
- Analyzer and all 93 Flutter tests pass, including focused rating eligibility, shared-cooldown, reveal, and verification-warning regressions. The production-configured iOS `1.0.2 (6)` release build is installed and standalone-launched on the owner's physical iPhone 17 Pro. On August 28 App Review returned that submission as **Unresolved Issues** under Guideline 4.3(a), using the generic binary/metadata/concept similarity notice; the prior iOS 1.0.0 submission had been approved but was cancelled without release. The signed Android `1.0.2 (6)` app bundle passed manifest, signature, source-freshness, and Google Play processing checks; its full production rollout is under **Changes in review** and will publish automatically after approval because managed publishing is off.

A 2026-07-30 design-craft pass (simulator-verified) added the Manrope brand type for headings/buttons, the light-blue MathSolver calculator icon used by math-solver.io, a viewfinder-bracket signature motif on the Home scan hero, frosted-glass floating navigation, a due-review badge on the Practice tab, labeled Watch/Try-it solution actions, true cross-quiz progress, entrance motion for revealed steps, haptics on shutter/reveal/answers, edge-fade hints on wide equations, dark-mode-safe mint/coral surfaces, correct status-bar contrast on app-bar screens, and real relative dates on the Recent card. A dev-only screenshot harness lives at `mobile_app/lib/dev/gallery_main.dart`.

The code-controlled launch product is substantially complete. The versioned gateway, private renderer, and Firestore rules reached production on 2026-07-28 with staged App Check validation. Native Google and Apple sign-in plus their Firebase/Apple provider configuration shipped on 2026-08-05. Android `1.0.0 (4)` is publicly installable and has passed a real Play Integrity-protected video request; current `1.0.2 (6)` adds the hardened restart/background/data-isolation work plus quiet milestone-based ratings and is in Google Play review for a full production rollout. Managed publishing is off, so approval will publish build 6 automatically. Apple `1.0.2 (6)` is blocked at **Unresolved Issues** by a Guideline 4.3(a) similarity rejection; the old approved 1.0.0 release was cancelled without publishing it. Remaining operational gates are resolving that review, the APNs `.p8`, broader App Check enforcement after legitimate-traffic validation, post-store-install regression, and Apple's manual release after approval. Widgets, Pencil scratchpad, exam packs, and Live Tutor remain later product phases rather than hidden launch claims.
...
- **Our video COGS is ~$0.02–0.15 per 60s** (storyboard LLM + deterministic render + TTS) — the exact 10/day verified-account boundary is the launch cost control; generative diffusion video stays irrelevant for equation-accurate steps.
...
- **Stack decision:** research initially favored Expo/React Native for a small JS team, but the owner selected Flutter for the shared iOS/Android client. The implemented foundation uses Flutter camera capture and native LaTeX; narrow Swift/Kotlin extensions remain later boundaries where PencilKit or on-device AI demand them.

### Business benchmarks

Historical market research found strong education-subscription conversion and a $9.99/month competitor cluster, but those benchmarks are not the current product strategy. MathSolver's approved launch is completely free. AI apps still churn ~30% faster than non-AI, so retention loops remain existential. US iOS CPI was estimated at $1.5–3.5, with Apple Ads median CPA around $2.76.
...
- **The pledge:** *Full step-by-step solutions. Free. Unlimited. No sign-up. No trial traps.* Every incumbent's worst reviews violate one of these; each clause is a conversion argument on comparison pages and the store listing.
- **Against incumbents:** they paywall understanding; MathSolver launches with no paid tier. Only video generation has a transparent 10-per-day product limit.
...
5. **Ask & Live Tutor.** Follow-up chat on any solution is free and reuses the existing `/api/solve`. A future **Live Tutor** could combine camera/voice with a synchronized whiteboard and the solver/verifier as tools, but it is not a launch feature or paid-plan commitment. *Why: the one seat still open in math-native live tutoring.*
...
- No community/forum (Brainly owns it; moderation cost). No human-tutor marketplace v1 (Gauth's high-ARPU layer — revisit at scale). No general-subjects expansion (essay/code/chem) — "math only, done best" *is* the positioning vs generalist chatbots and wrapper apps. No ad SDK ships at launch; any future ad release requires a fresh product, privacy, consent, and store-policy review. No AR page-overlay gimmick — stabilized viewfinder + fast results beats it; Android OS already commoditized live overlay.

## 5. UX architecture

**Three destinations:** **Home** (one dominant Scan action + inline type/photo/paste + recent work) · **Library** (segmented solutions/videos with search) · **Practice** (daily warm-up, due reviews, quizzes, exam packs later). Account, progress, privacy, and appearance sit behind the persistent avatar; they do not deserve a full-time tab.

**First-run:** one value-dense screen (Scan → Understand → Watch) → Home. Camera permission is contextual and appears only after Scan; denying it does not block type/photo/paste. Account remains optional (guest = device-local, exactly like web); native Apple, Google, and verified Email/Password sign-in feed the verified-owner Firestore notebook merge. The free launch has no paywall or purchase flow.
...
**Video states:** generation is async (30–90s). The mobile waiting state uses a visibly animated storyboard/playhead, a determinate progress track, and the honest Plan → Narrate → Animate → Finish pipeline instead of a generic AI sparkle card; the exact source problem stays visible and the user can leave while the private job continues into Library. Showing playable storyboard scenes before the final continuous MP4 remains future streaming work rather than a fake loading claim.

**Edge states designed, not defaulted:** unreadable scan (guided retake with framing tips), no network (offline notebook + cached videos + on-device tier later), quota reached (state when the 10/day allowance resets, with no purchase prompt), verification failed (show the disagreement honestly, offer re-check with stronger model).

## 6. Technical plan (reuse-first)

- **Client:** Flutter/Dart in the isolated `mobile_app/` directory. The shipped client uses camera/image crop, streaming HTTP, native LaTeX, native Firebase Auth session persistence, local-first persistence, Firestore REST sync, App Check, Analytics, Messaging, video playback/offline cache, and native sharing behind a light-first adaptive Material 3 UI. Platform code remains narrow; there is no billing SDK. Widgets, PencilKit, and on-device AI remain later boundaries.
...
- **Video:** the existing render farm remains authoritative. Mobile starts one deterministic server job, suspends only polling while inactive, and resumes/reopens that same job after screen-off or process recreation. It adds explicit per-account push-on-ready consent, owner-scoped private offline caching, playback controls, and native share export. FCM tokens are server-only and the renderer receives only a narrow `cloudmessaging.messages.create` role.
- **Cost envelope per active user/day (order of magnitude):** solve+verify $0.001–0.01; video generation remains the material cost driver and is bounded at 10 generations per verified account per UTC day.

## 7. Launch business strategy

- **Completely free launch:** unlimited scans, full steps, follow-up chat, OCR, Check My Work, practice, mistake review, and notebook sync. There are no subscriptions, purchases, paid plans, credits, trials, or monetization prompts.
- **Video quota:** every verified account receives exactly 10 video generations per UTC day. Failed generation attempts refund the same-day reservation. The quota is a product/cost boundary, not a teaser for a paid upgrade.
- **Possible future ads:** ads may be evaluated after acquisition and retention are understood. They are not part of v1 and adding them alone must never activate the minimum-version gate.
- **Update contract:** ordinary versions may show a dismissible recommendation once per target version. A server-controlled hard gate is allowed only for an explicitly declared security or incompatible-protocol minimum version with a valid store destination.
...
- **Audience:** normal Education listing for math students and general learners, including grades 5–6. Do not position or submit the app as a Kids Category product, and do not add a blocking 13+ or 18+ gate. Answer store age-rating questions from the content actually present in the app.
...
| **1 — MVP** | Android live; build 6 in both reviews | Camera/crop/worksheet solve, hint-first verified steps, follow-up chat, private video with offline/share/optional push, guest→Firestore sync, practice + spaced mistake review, warm-up, in-app deletion, and both iOS/Android hosts. Android `1.0.2 (6)` is in full-production review; iOS `1.0.2 (6)` has a Guideline 4.3(a) unresolved issue. |
...
Activation: install→first solved <60s median; install→first video ≥35%. Loop health: D1 ≥ 35%, D7 ≥ 15% (school weeks), solves/session ≥3, video→practice continuation ≥25%. Trust: wrong-answer reports <5/1,000 solves; verification-pass rate visible internally. **Re-plan triggers:** D7 <8% after two school-month cohorts (loop is broken — stop adding features, fix retention); video watch-through <40% (format wrong); CPI-payback >12 months on paid channels (go organic-only).
