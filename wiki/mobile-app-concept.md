...
The code-controlled launch product is substantially complete. The versioned gateway, private renderer, and Firestore rules reached production on 2026-07-28 with staged App Check validation. Native Google and Apple sign-in plus their Firebase/Apple provider configuration shipped on 2026-08-05. Remaining owner-controlled release gates are distribution signing, an APNs `.p8` upload for iOS notification delivery, store records/listings, Android release SHA registration, real-device QA, and switching App Check from observation to mandatory after valid traffic is measured. RevenueCat products need provider/store business configuration; widgets, Pencil scratchpad, exam packs, and Live Tutor remain later product phases rather than hidden MVP claims.
