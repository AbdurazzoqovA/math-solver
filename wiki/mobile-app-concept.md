...
The code-controlled launch product is substantially complete. The versioned gateway, private renderer, and Firestore rules reached production on 2026-07-28 with staged App Check validation. Native Google and Apple sign-in plus their Firebase/Apple provider configuration shipped on 2026-08-05, and the App Store record, metadata, privacy label, free worldwide availability, and final screenshots are configured without submission. Remaining owner-controlled release gates are distribution signing, an APNs `.p8` upload for iOS notification delivery, Android release SHA registration, physical-device flow QA, and switching App Check from observation to mandatory after valid traffic is measured. Widgets, Pencil scratchpad, exam packs, and Live Tutor remain later product phases rather than hidden launch claims.
...
| **1 — MVP** | code-complete; store gates pending | Camera/crop/worksheet solve, hint-first verified steps, follow-up chat, private video with offline/share/optional push, guest→Firestore sync, practice + spaced mistake review, warm-up, in-app deletion, and both iOS/Android hosts. Store signing/listing and device QA remain. |
