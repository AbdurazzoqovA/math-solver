...
- **Mobile:** standalone Flutter 3.44/Dart 3.12 iOS/Android client in `mobile_app/`, publicly available on the App Store and Google Play; never mix its source into the Next.js root. The App-Check-aware `/api/mobile/v1/*` client includes camera crop/worksheet capture, Check My Work, verified streamed steps, practice/review, native Email/Password + Google/Apple auth, Firestore notebook sync, private visual lessons with offline/share/push, and opt-in no-content analytics. Billing is deliberately absent; APNs setup, broader App Check enforcement, and later device-only surfaces remain operations/roadmap work.
- **Solver:** switchable Azure **gpt-4.1-nano** / Google **Gemini 3.1 flash-lite** (`LLM_PROVIDER`) — `src/app/api/solve/route.ts`, streamed through `src/lib/llm.ts`. Prompt `MATH_TUTOR_PROMPT` forces `**Step N:**` format.
- **OCR (photo/PDF/drawing → text):** the selected Azure/Gemini LLM — `src/app/api/ocr/route.ts`. Extracts the expression only; does not solve.
- **Practice quizzes:** the selected Azure/Gemini LLM — `src/app/api/practice/route.ts` + `/steps`.
- **Interactive visual lessons:** verified accounts get five private, asynchronous micro-lessons from the latest completed solution — `src/app/api/video/jobs`, `src/components/video`, and `src/lib/video`. Cloud Tasks calls the private `services/video-renderer` Cloud Run worker, which uses typed Gemini plan/review (independent `VIDEO_LLM_PROVIDER`), switchable Azure/Gemini TTS with Gemini transcription QA, Manim/FFmpeg, and private signed GCS playback.
...
- **Analytics:** Website GA4 (`G-YG1NPYM8BS`) loads without an in-product consent popup; mobile analytics are opt-in. The no-content event adapter lives in `src/lib/analytics.ts`. Never attach math content, identity fields, or notebook IDs.
